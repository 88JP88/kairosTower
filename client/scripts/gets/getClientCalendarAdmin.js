...
const parametro1 = urlObj.searchParams.get("clientId");


getCalendarDays1(parametro1);
}

async function getCalendarDays1(param) {
  ...
  fetch(epGetCalendarDays + param+"/admin")
          ...
          data.calendarDays.forEach(info => {
             ...
             card.innerHTML = `
                  <div class="card-body" style="background-color: ${backgroundColor};">
                      ...
                      ${info.isActive !== "0" ? `<button onclick="openModCalendarDaysAssignAdmin();getCalendarDaysAssign1(&quot;${info.calendarId}&quot;);" class="btn btn-primary1 edit-button" title="DÍAS DISPONIBLES"><i class="fas fa-calendar-alt"></i></button>` : ''}
                  </div>
              `;
...
async function getCalendarDaysAssign1(param) {
  

  document.getElementById("loading-container").style.display = "flex";
  sessionStorage.setItem('calendarNow',param);
  fetch(epGetCalendarDaysAssign + param+"/admin")
      .then(response => response.json())
      .then(data => {
          const cardContainer1 = document.getElementById("card-container123");
          ...
          data.calendarDaysAssign.forEach(info => {
              ...
              card1.innerHTML = `
                  <div class="card-body"  style="background-color: ${backgroundColor};">
                      ...
                      ${info.isActive !== "0" ? ` <button onclick="openModCalendarTime();getCalendarTime1(&quot;${info.registId}&quot;);" class="btn btn-primary1 edit-button" title="HORAS DISPONIBLES"><i class="fas fa-clock"></i></button>` : ''}
                      </div>
              `;
...
async function getCalendarTime1(param) {
  document.getElementById("loading-container").style.display = "flex";
  sessionStorage.setItem('registNow',param);
  fetch(epGetCalendarTime + param+"/admin")
      .then(response => response.json())
      .then(data => {
          ...
          data.calendarTime.forEach(info => {
              ...
              card11.innerHTML = `
                  <div class="card-body" style="background-color: ${backgroundColor};">
                      ...
                      <p class="card-text">${info.isActive == "1" ? ` ${info.counterId > 0 ? `<i class="fas fa-check-circle"></i> <button onclick="openModUserTimeAdmin();getInternalUserList1(&quot;all&quot;);getClientRoomsList1(&quot;${info.timeId}&quot;);" class="btn btn-primary1 edit-button" title="ASIGNAR USUARIO"><i class="fas fa-user-plus"></i></button><button onclick="openModUserTimedesadmin();getCalendarTimedes1(&quot;${info.timeId}&quot;);" class="btn btn-primary1 edit-button" title="DESASIGNAR USUARIO"><i class="fas fa-unlink"></i></button>` : `<i class="fas fa-times-circle"></i><button onclick="openModUserTimedes();getCalendarTimedes(&quot;${info.timeId}&quot;);" class="btn btn-primary1 edit-button" title="DESASIGNAR USUARIO"><i class="fas fa-unlink"></i></button>`}` :''}</p>
                      <p class="card-text">Usuario: ${info.userApply}</p>
                      ${info.notApply !== "free" ? ` <button onclick="openModCalendarDaysAssignAdmin();getCalendarDaysAssign1(&quot;${info.timeId}&quot;);" class="btn btn-primary1 edit-button"><i class="fas fa-info-circle"></i></button>` : ''}
                  </div>
              `;
...
async function getClientRooms1() {
  document.getElementById("loading-container").style.display = "flex";
          ...
          data.clientRoom.forEach(info => {
              ...
              card11.innerHTML = `
                  ...
                  <p class="card-text"><div class="edit-container"><input type="text" class="form-control label-input" id="${info.clientId}" value="${info.comments}" title="${info.comments}"> <button onclick="editClientRoom(this,&quot;${info.roomId}&quot;,&quot;comments&quot;,&quot;comments&quot;,&quot;0&quot;,&quot;${info.clientId}&quot;)" class="btn btn-primary1 delete-button" title="EDITAR"><i class="fas fa-edit"></i></button></div></p>
                      ...
                      <div id="checkbox-roomid22${info.roomId}" class="card-container">
                      ...
                      <div id="checkbox-roomid11${info.roomId}" class="card-container">
                      <!-- Contenido de la sección expandible -->
                      </div>
                      <div class="edit-container">
                      <button onclick="assignelementbyuserroomadmin(&quot;${info.roomId}&quot;,&quot;${info.roomId}&quot;,&quot;${info.roomId}&quot;)" class="btn btn-primary1 delete-button" title="VERIFICAR">
              ...
              `;
              getClientElemntCheck1(info.roomId,'usedbyclient',info.roomId,info.roomId);
              getClientElemntCheck1(info.roomId,'notusedbyclient',info.roomId,info.roomId);
              cardContainer11.appendChild(card11);
          });
          document.getElementById("loading-container").style.display = "none";
      })
...
async function getCalendarTimedes1(param) {
  document.getElementById("loading-container").style.display = "flex";
  ...
  fetch(epGetCalendarTimedes + param)
      .then(response => response.json())
      .then(data => {
          const cardContainer11 = document.getElementById("card-usertimedesadmin");
          cardContainer11.innerHTML = ""; // Borra las tarjetas antiguas
...
<div id="checkbox-des1${info.assignId}" class="card-container">
<!-- Contenido de la sección expandible -->
</div>
...
<p class="card-text">
          ...
          <button onclick="assignDesadmin(&quot;${info.assignId}&quot;)" class="btn btn-primary1 delete-button" title="DESASIGNAR ROOM">
            <i class="fas fa-trash"></i>
          </button>
        </div>
                      </p>
              ...
              `;
              getClientElemntCheckdes1(info.assignId,info.roomId);
             // getClientElemntCheck1(info.roomId,'notassign',info.userId,info.assignId);
              cardContainer11.appendChild(card11);
          });
          document.getElementById("loading-container").style.display = "none";
      })
...
function createCheckbox31(info) {
  const checkbox = document.createElement("input");
  checkbox.type = "checkbox";
  checkbox.name = "assignCheckboxelementbyadmin1"; // Asigna un nombre a los checkboxes
  checkbox.value = info.elementId; // Asigna un valor (puedes usar un identificador único)
  checkbox.addEventListener("change", handleCheckboxChangeAssignElementbyuser1); // Agrega un manejador de eventos para el cambio
...
function createCheckbox41(info) {
  const checkbox = document.createElement("input");
  checkbox.type = "checkbox";
  checkbox.name = "assignCheckboxelementbyadminnot1"; // Asigna un nombre a los checkboxes
  checkbox.value = info.elementId; // Asigna un valor (puedes usar un identificador único)
  checkbox.addEventListener("change", handleCheckboxChangeAssignElementbyusernot1); // Agrega un manejador de eventos para el cambio
...
async function getClientElemntCheck1(filter,param,ids,ids1) {
...
if(param=="assign"){
  ...
  // Obtener el valor del parámetro "parametro1"
  var param = urlObj.searchParams.get("clientId");
  //var param=sessionStorage.getItem('clientNow');
    ...
    if(param="usedbyclient"){
      ...
      fetch(epGetClientElements + param+"/usedbyclient/"+filter+"/"+ids+"/"+ids1)
          ...
          .then(data => {
              ...
              const checkboxContainer = document.getElementById("checkbox-roomid22"+filter);
            
              checkboxContainer.innerHTML = ""; // Borra los checkboxes antiguos
    
              data.clientElement.forEach(info => {
                  const checkbox = createCheckbox31(info);
                  checkboxContainer.appendChild(checkbox);
        ...
        if(param="notusedbyclient"){
          ...
          fetch(epGetClientElements + param+"/notusedbyclient/"+filter+"/"+ids+"/"+ids1)
              ...
              .then(data => {
                  ...
                  const checkboxContainer = document.getElementById("checkbox-roomid11"+filter);
                
                  checkboxContainer.innerHTML = ""; // Borra los checkboxes antiguos
        
                  data.clientElement.forEach(info => {
                      const checkbox = createCheckbox41(info);
                      checkboxContainer.appendChild(checkbox);
                      console.log("ejecutado usado");
                  });
...
// Función para ejecutar al cambiar la selección en el select
function onClientRoomSelect1(param) {
  const selectElement = document.getElementById("list-clientroom1");
  ...
  if (selectedValue) {
    ...
    getClientElemntCheck1(selectedValue,param,"1","1");
...
function createCheckboxdes1(info) {
  const checkbox = document.createElement("input");
  checkbox.type = "checkbox";
  checkbox.name = "assignCheckbox1"; // Asigna un nombre a los checkboxes
  checkbox.value = info.elementId; // Asigna un valor (puedes usar un identificador único)
  checkbox.addEventListener("change", handleCheckboxChangedes1); // Agrega un manejador de eventos para el cambio
...
async function getClientElemntCheckdes1(filter,param) {
  document.getElementById("loading-container").style.display = "flex";
  ...
  fetch(epGetClientElements + param+"/hold/"+filter+"/1/1")
      .then(response => response.json())
      .then(data => {
          const checkboxContainer = document.getElementById("checkbox-des1"+filter);
          checkboxContainer.innerHTML = ""; // Borra los checkboxes antiguos

          data.clientElement.forEach(info => {
              const checkbox = createCheckboxdes1(info);
              checkboxContainer.appendChild(checkbox);
...
function handleCheckboxChangedes1(event) {
    const assignId = event.target.value;
...
function handleCheckboxChangeAssignElementbyuser1(event) {
  const assignId = event.target.value;
...
function handleCheckboxChangeAssignElementbyusernot1(event) {
  const assignId = event.target.value;
