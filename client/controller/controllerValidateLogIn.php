...
$url = $sub_domain . "/kairosGateway/apiCore/v1/validateLogIn/6WclAmsaP9H7SR2WmpDbl1OL9";
...
$data = json_decode($_SESSION['userInfo']);
  ...
  //  $_SESSION['userName'] =$character->userName;
    ...
    $_SESSION['clientNameResponse'] =$character->clientName;
    ...
    $_SESSION['styleId'] =$character->styleId;
...
if (strtolower($_SESSION['response']) === "true") { // Convertir la respuesta a minúsculas antes de comparar
  ...
  header ('Location: ../k_'.$_SESSION['clientNameResponse'].'/session.php');
}
...
elseif (strtolower($_SESSION['response']) === "false") { // Convertir la respuesta a minúsculas antes de comparar
    ...
    header ('Location: ../k_'.$_SESSION['clientNameResponse'].'/index.php');
}
