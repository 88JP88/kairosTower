...
function assignelementbyuserroomdes(room,user,assign) {
...
var selectedAssignmentsString = selectedAssignmentselementbyuser.join("|"); // Convierte el array en una cadena con valores separados por "|"
  ...
  var url = "controller/postAssignRoom.php?" +
            ...
            "&param=" + encodeURIComponent('asigelementroomdes')+
  ...
  fetch(url)
    .then(response => {
      ...
      getMessage();
      getClientElemntCheck(room,'usedbyclient',room,room);
      getClientElemntCheck(room,'notusedbyclient',room,room);
    })
    .catch(error => {
